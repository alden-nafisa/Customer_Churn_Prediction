      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[550px]">
          <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
            ...
            <div className="flex bg-white border border-slate-200 p-1 rounded-lg shadow-sm">
              {['All', 'Churned', 'Not Churned'].map(filter => (
                <button key={filter} onClick={() => setChurnFilter(filter)} className={`text-[10px] font-bold px-3 py-1.5 rounded-md transition-all ${churnFilter === filter ? 'bg-indigo-50 text-indigo-700' : 'text-slate-400 hover:text-slate-600'}`}>{filter}</button>
              ))}
            </div>
          </div>
          <div className="px-5 py-3 grid grid-cols-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50">
            <span>Customer ID</span><span className="text-right">Details</span>
          </div>
          <div className="flex-1 overflow-y-auto p-2">
            {filteredChurn.map((customer, idx) => (
              <div key={idx} className="flex items-center justify-between p-3.5 hover:bg-slate-50 rounded-xl transition-colors group cursor-pointer">
                <div className="flex items-center gap-4">
                  <div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center border border-indigo-100 text-indigo-500 shadow-sm"><User size={16} /></div>
                  <div>
                ...
                <div className="text-right flex flex-col items-end gap-1">
                  ...
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${customer.status === 'Churned' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>{customer.status}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[550px]">
          <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
            ...
            <div className="flex bg-white border border-slate-200 p-1 rounded-lg shadow-sm">
              {['All', 'Positive', 'Negative', 'Netral'].map(filter => (
                <button key={filter} onClick={() => setFeedbackFilter(filter)} className={`text-[10px] font-bold px-3 py-1.5 rounded-md transition-all ${feedbackFilter === filter ? 'bg-indigo-50 text-indigo-700' : 'text-slate-400 hover:text-slate-600'}`}>{filter}</button>
              ))}
            </div>
          </div>
          <div className="px-5 py-3 grid grid-cols-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50">
                ...
                <div key={idx} className="flex items-start justify-between p-3.5 hover:bg-slate-50 rounded-xl transition-colors group cursor-pointer gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] font-medium text-slate-500 truncate">{feedback.text}</p>
                    </div>
                  </div>
          ...
          <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-b-2xl">
            <button onClick={() => setActiveTab('sentiment')} className="bg-indigo-500 hover:bg-indigo-600 text-white text-[11px] font-bold px-5 py-2 rounded-lg shadow-sm transition-colors">All Feedback</button>
            <span className="text-[11px] font-medium text-slate-500">1,520 Total Feedbacks</span>
          </div>
        </div>
      </div>
    </div>
